...
\\
Parameters which are performing well in above experiments withtest functions are used to check the rate of convergence for Schwefel function shown in figure \ref{fig:schwefel} this is another test function for which only objective function with iteration is studied as in figure \ref{fig:con_schwefel}. the aim to do this study is to get a combination of optimzation parameters such that a better rate of convergence can be achieved so that a good optimizaed resuklt can be obtaoined with in less number of iterations. this can be shown in figure \ref{fig:con_schwefel} that for higher value if inertia does not show convergence, low value of inertia have high rate of convergence but doent reach to the optimized value but 0.6 shows a good convergence as well as with differenct \(c_1\) and \(c_2\) it convergeces. this analysis shows that \(c_1\) at 1.8, and \(c_2\) at 1.8 amd 2.4 is converges to zero but the convergence is slow but \(c_1\) at 2.4 and \(c_2\) rnaging from 1.2 to 1.8 is showing good rate of convergence and reach to near to zero within 100 iterations. with this analyis optimization parameters are set inertia 0.6, \(c_1\) at 2.4, and \(c_2\) at 1.2 to 1.8.
%\begin{figure}	
