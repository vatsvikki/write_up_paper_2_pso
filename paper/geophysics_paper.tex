...
\\
We propose a technique to define a depth-velocity profile at sparsely horizontal positions with the depth of interfaces (\(c_i\)), where \(i\) is representing the \(i^{th}\) interface, rate of change (\(g_i\)) of velocity between \((i - 1)^{th}\) to \(i^th\) interface, and velocity (\(v_i\)) of model below the \(i^th\) interface,  These multiple depth-velocity profiles sparasly located are interpolated and smoothened for preparing an initial model. it is also assumed that two interface donot intersect each other whihc is gelological not possible. 
\section*{Theory}
