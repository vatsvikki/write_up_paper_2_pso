...
In this implimentation of this inversion strategy for preparing intial mode we have sparsaly select the shot locations and invert the near offset trace to get velocity-depth profile for that particular shot. this process is repeated for multiple shots and for inverting these shots major seismic peaks are selected from near offset seismic trace which are most probably due to the reflectors just beneath the shot location, these peaks are selected to guide the optimization for inverting the data to get the major interfaces. we have already provide a flexiblity to the time of the selected peak because its not always that much acccurate to select the peak. Initially this inversion is started with a uniform velocity same as the upper layer, and optimization is strated progressively for with uppermost peak, the inverted model for this particular peak serve as an initial model for inversion of next peak.  there may be confusion with the peaks psicking between the reflection and multiples  but this model is not modified with the inversion of multiples becuase these multiples are due to the reflector which are already justifed.
