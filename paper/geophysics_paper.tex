...
\\
We propose a technique to define model with depth-velocity profile at sparse horizontal positions, as illustrated in Figure \ref{fig:parameterization.jpg}. This model represents a layered marine environment, where the velocity gradually increases with depth, ranging from 1500 to 4700 m/s. Additionally, it features a low-velocity layer situated between two high-velocity layers. The depth of interfaces (\(c_i^k\)), where \(i\) is representing the \(i^{th}\) interface for \(k^{th}\)depth-velocity profile, (\(g_i^k\)) is rate of change of velocity between \((i - 1)^{th}\) to \(i^th\) interface, and velocity (\(v_i^k\)) of model at \(i^th\) interface,  These multiple depth-velocity profiles sparasly located are interpolated for preparing an initial model. it is also assumed that two interface donot intersect each other whihc is gelological not possible. we have also bounded the search space with geologically possible physical parameters. PSO focuses to optimize on these three parameters for every layer for preparing an initial model.
